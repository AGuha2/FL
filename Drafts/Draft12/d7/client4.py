...
# X stays flat 2D (samples, NUM_FEATURES) here. Any model-specific reshaping
# (e.g. the CNN variant's Conv1D input) now happens inside model.py itself,
# so swapping architectures never requires touching this file.
...
# NOTE: the previous version used the default reduction (auto-averaged to a
# scalar), so multiplying by per-sample class weights afterward wasn't
# actually applying weights per sample — reduction=NONE fixes this to
# return true per-sample losses, which both the class weighting and the
# focal loss modulation below require to work correctly.

FOCAL_GAMMA = 1.0  # focal loss focusing parameter — down-weights easy/
                    # majority-class examples so gradient signal
                    # concentrates on hard/rare-class examples, which
                    # plain class weighting alone doesn't do.
...
def train_one_epoch_fedprox(model, x_train, y_train, class_weight_dict,
    ...
    for xb, yb, swb in make_batches(x_train, y_train, batch_size, class_weight_dict):
        ...
        with tf.GradientTape() as tape:
            ...
            per_sample_ce = loss_fn(yb, preds)
            # Focal modulation: (1 - p_t)^gamma, where p_t is the model's
            # predicted probability for the TRUE class. Easy/confident
            # correct predictions (p_t near 1) get down-weighted toward 0;
            # hard/rare examples (p_t near 0, i.e. still being misclassified)
            # keep close to full loss weight.
            p_t = tf.gather(preds, yb, axis=1, batch_dims=1)
            focal_factor = tf.pow(1.0 - p_t, FOCAL_GAMMA)
            per_sample_loss = per_sample_ce * focal_factor
...
class FlowerClient(fl.client.NumPyClient):
    ...
    def fit(self, parameters, config):
        ...
        # DeepExplainer is viable again now that the model is pure CNN (no
        # LSTM) — its op-override rules cover Conv1D/BatchNorm/MaxPool/Dense
        # cleanly. It's also cheaper per-class than GradientExplainer, so we
        # can afford to explain all 34 classes instead of a top-k subset.
        ...
        if mean_abs_shap.ndim > 1:
            mean_abs_shap = mean_abs_shap.mean(axis=-1)            # collapse any extra channel dim -> (features,)
        ...
        # Reserved key, not a feature — lets the server identify this client
        # by its real CLIENT_ID instead of guessing from response order
        # (Flower's results list order isn't guaranteed to be the same
        # client round-to-round). The server strips this key back out
        # before treating the rest of the dict as feature importances.
        shap_metrics["__client_id__"] = CLIENT_ID
