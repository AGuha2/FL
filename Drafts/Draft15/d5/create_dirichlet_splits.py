...
SEED         = 42
TEST_FRACTION = 0.10
# NOTE: MAX_ROWS cap removed — with ~3M rows in the 90% client pool split
# across 5 clients, a 300k cap was silently discarding ~40-50% of the data.
# If you later need a cap for memory/runtime reasons, reintroduce it here
# and set it well above the largest expected per-client share.
np.random.seed(SEED)

# ── Stratified 90/10 split: 10% held out as server-side test set ──────────────
# stratify=df['Label'] guarantees every class present in df appears in both
# the server test set and the client pool, in roughly the same proportions.
...
print(f"\nClient pool (90%): {len(client_pool_df):,} rows")
print(f"Server test (10%): {len(server_test_df):,} rows")

server_test_path = r'D:\CAPSTONE\Server_Test.csv'
server_test_df.to_csv(server_test_path, index=False)
print(f"✓ Server test set saved → {server_test_path}")

# From here on, `df` refers only to the 90% client pool — the Dirichlet
# partitioning below never sees the held-out server test rows.
df = client_pool_df

#Fit ONE global scaler on the client pool (90%) only, to avoid leaking
# any information from the held-out server test set.
scaler = RobustScaler()
...
print(f"\nUnique classes in client pool: {len(classes)}")
