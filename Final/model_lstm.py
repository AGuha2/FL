...
def build_model(
    ...
    """Combine the successful single CNN block with a two-LSTM tail.
    Each existing tabular client input remains shaped ``(batch, features)``.
    The internal reshape exposes the feature axis to Conv1D and LSTM without
    requiring changes to the scaler, clients, server, or SHAP feature names.
    """
    inputs = keras.Input(shape=(num_features,), name="network_features")
    x = layers.Reshape((num_features, 1), name="feature_sequence")(inputs)
    # Single CNN block retained from the better-performing CNN experiment.
    x = layers.Conv1D(
        128,
        kernel_size=3,
        padding="same",
    ...
    # Recurrent tail adapted from the supplied architecture.
    x = layers.LSTM(
        128,
        activation="tanh",
