...
# Conv1D expects (samples, steps, channels); each feature row becomes a
# length-NUM_FEATURES, 1-channel "signal".
X = X.reshape(X.shape[0], NUM_FEATURES, 1)
...
class FlowerClient(fl.client.NumPyClient):
    ...
    def fit(self, parameters, config):
        ...
        mean_abs_shap = np.mean(np.abs(shap_array), axis=(0, 1))  # (features, channels)
        mean_abs_shap = mean_abs_shap.mean(axis=-1)                # collapse channel -> (features,)
