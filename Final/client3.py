"""Seven-class FedAvg baseline client 3."""
import json
import random
import sys
...
CLIENT_ID = 3
DATA_PATH = rf"D:\CAPSTONE\IoT6Bal_Client{CLIENT_ID}.csv"
...
def stratified_shap_indices(targets, samples_per_class, seed):
...
def mean_abs_shap(model, background, sample, number_of_features):
    ...
    if array.ndim == 3 and array.shape[1] == number_of_features:
        ...
        # (outputs, samples, features) or (samples, outputs, features)
...
shap_sample_indices = stratified_shap_indices(
    y_train,
...
class FedAvgClient(fl.client.NumPyClient):
    ...
    def fit(self, parameters, config):
        ...
        return model.get_weights(), len(x_train), {
            ...
            **{
                f"shap::{feature_name}": float(shap_value)
                for feature_name, shap_value
                in zip(feature_names, shap_vector)
            },
        }
...
if len(sys.argv) < 2:
    raise ValueError("Usage: python client3.py <port>")
