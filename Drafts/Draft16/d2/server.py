...
# Server validation supplies the previous round's class F1. The separate
# final test partition is used only after the last aggregation.
...
class ClassSpecificReliableShap(fl.server.strategy.FedAvg):
    """FedAvg blended with leave-one-out median SHAP agreement."""
    ...
    def aggregate_fit(self, server_round, results, failures):
        ...
        print(
            f"[Consensus SHAP + previous server F1] R{server_round}: "
            + ", ".join(
