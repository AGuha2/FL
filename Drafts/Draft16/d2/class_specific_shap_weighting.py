"""Class-specific consensus-SHAP aggregation utilities.
...
def calculate_client_weights(
    ...
    *,
    ...
    """Combine FedAvg with leave-one-client-out median SHAP agreement.

    The previous round's server validation F1 does not score clients
    directly. It determines which classes deserve more attention. For each
    client and class, SHAP similarity is measured against the element-wise
    median SHAP vector from the other clients where that class is available.
    """
    ...
    if not np.all(np.isfinite(client_shap)):
        raise ValueError("Client SHAP contains non-finite values.")
    ...
    weighted_comparable = comparable * importance[None, :]
    score_denominator = weighted_comparable.sum(axis=1)
    score = np.divide(
        np.sum(
            similarities * weighted_comparable,
            axis=1,
        ),
        score_denominator,
