...
df = pd.read_csv(r'D:\CAPSTONE\NonIID_Client3.csv')
...
class FlowerClient(fl.client.NumPyClient):
    ...
    def evaluate(self, parameters, config):
        ...
        for i in sorted_idx:
            print(f"{feature_names[i]:<30} {mean_abs_shap[i]:>12.6f}")
        ...
        plt.title(f"Client {self.client_id} SHAP Summary (Global Model)")
        plt.tight_layout()
        plt.savefig(f"client_{self.client_id}_shap_round_{round_num}.png")
        plt.close()

        print(f"[Client {self.client_id}] SHAP summary saved → client_{self.client_id}_shap_round_{round_num}.png")
        ...
        shap.summary_plot(
            ...
            plot_type="bar",        # <-- this gives the importance bar chart
            max_display=20,
            show=False
        )
        plt.title(f"Client {self.client_id} Feature Importance (Round {round_num})")
        plt.tight_layout()
        plt.savefig(f"client_{self.client_id}_importance_round_{round_num}.png")
        plt.close()
...
fl.client.start_numpy_client(
        ...
        client=FlowerClient(client_id=3) , 
