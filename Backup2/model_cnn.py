...
def build_model(input_features, num_classes, output_activation='softmax'):
    """
    CNN architecture, adapted from the provided model diagram: LSTM layers
    removed (see below), and trimmed down to a single Conv block instead of
    two.
    ...
    WHY TRIMMED TO ONE CONV BLOCK
    ------------------------------
    With only ~39 input features, two Conv1D+MaxPool stages downsample
    aggressively (39 -> ~18 -> ~8) before reaching the dense layers, leaving
    very little spatial extent for the second block's kernel to work with.
    One block (39 -> ~18) keeps more signal, still tests whether convolution
    helps on this data, and removes the larger share of the model's compute
    cost (Conv blocks cost more than the Dense tail), directly addressing
    both slower-than-expected training and excess model depth relative to
    the feature count.
