"""Pure sample-size FedAvg baseline with seven-class evaluation."""
import json
import sys
...
RESULT_DIR = Path("results_10round_7class_equal_reliable_shap")
...
class ReliableShapFedAvg(fl.server.strategy.FedAvg):
    """FedAvg with a bounded, validation-gated 10% SHAP adjustment."""
    ...
    def aggregate_fit(self, server_round, results, failures):
        ...
        # SHAP is intentionally bounded to 10%; FedAvg remains the anchor.
        final_weights = normalized(
            0.90 * size_weights
            + 0.10 * reliable_shap_weights
        )
        ...
        for record, client_weight in zip(records, final_weights):
            ...
            aggregation_history.append({
                ...
                "fedavg_size_weight": float(
                    size_weights[record["client_id"] - 1]
                ),
                "shap_alignment": float(
                    alignments[record["client_id"] - 1]
                ),
                "server_validation_f1": float(
                    validation_f1[record["client_id"] - 1]
                ),
                "reliable_shap_weight": float(
                    reliable_shap_weights[record["client_id"] - 1]
                ),
                "final_weight": float(
                    final_weights[record["client_id"] - 1]
                ),
            })
